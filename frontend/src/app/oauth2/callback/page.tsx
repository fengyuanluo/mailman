...
export default function OAuth2CallbackPage() {
    ...
    // 从URL路径中提取provider参数
    // URL格式：/oauth2/callback/gmail 或 /oauth2/callback/outlook
    const pathSegments = window.location.pathname.split('/')
    const provider = pathSegments[pathSegments.length - 1] as OAuth2ProviderType
    ...
    const handleCallback = async () => {
        try {
            ...
            if (!provider) {
                setError('缺少 provider 参数，请检查回调URL是否包含 ?provider=gmail')
                setStatus('error')
                return
            }
